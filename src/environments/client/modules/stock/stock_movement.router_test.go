...
func TestNewStockMovementRouter(t *testing.T) {
		...
		t.Run("Should success on", func(t *testing.T) {
			...
			for _, testCase := range testCases {
				t.Run(fmt.Sprintf("Method: %s Path: %s Handler: %s", testCase.Method, testCase.Path, testCase.Handler), func(t *testing.T) {
					...
					mockStockMovementC := new(mock.ClientStockMovementController)
					NewStockMovementRouter(subRouter, mockStockMovementC)
					mockStockMovementC.On(
						testCase.Handler,
					...
					mockStockMovementC.AssertExpectations(t)
					mockStockMovementC.AssertNumberOfCalls(t, testCase.Handler, 1)
