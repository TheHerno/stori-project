...
func TestGormRepository(t *testing.T) {
	t.Run("FindByID", func(t *testing.T) {
		...
		t.Run("Should success on", func(t *testing.T) {
			...
			got, err := rWarehouse.FindByID(id)
	...
	t.Run("FindAndLockByID", func(t *testing.T) {
		t.Run("Should success on", func(t *testing.T) {
			// fixture
			WarehouseIDToFind := warehouses[1].WarehouseID
			...
			go func() {
				got, err := rWarehouse.FindAndLockByID(WarehouseIDToFind)
			...
			go func() {
				err2 := connection.Table("warehouse").Where("warehouse_id = ?", WarehouseIDToFind).
		...
		t.Run("Should fail on", func(t *testing.T) {
			...
			t.Run("Table doesn't exist", func(t *testing.T) {
				...
				tx := connection.Begin()
				rWarehouse := NewWarehouseGormRepo(tx)
				tx.Unscoped().Where("1=1").Delete(&entity.Warehouse{}) //Cleaning movements
				tx.Migrator().DropTable(&entity.Warehouse{})

				got, err := rWarehouse.FindAndLockByID(1)
